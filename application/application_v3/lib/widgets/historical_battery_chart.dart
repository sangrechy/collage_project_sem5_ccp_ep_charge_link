...
class HistoricalBatteryChart extends StatelessWidget {
  const HistoricalBatteryChart({
    super.key,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                const Icon(Icons.battery_charging_full_rounded, size: 20, color: AppColors.green),
                const SizedBox(width: 8),
                const Text(
                  'BATTERY PERCENTAGE PROFILE',
                  style: TextStyle(
                    fontSize: 12,
                ...
                if (points.isNotEmpty)
                  Text(
                    '${points.last.batteryPercentage}%',
                    style: AppTheme.mono(size: 14, weight: FontWeight.w700, color: AppColors.green),
                  ),
              ],
            ),
            ...
            if (points.isNotEmpty) ...[
              Wrap(
                ...
                children: [
                  _StatBadge(
                    label: 'MIN',
                    value: '${points.map((p) => p.batteryPercentage).reduce((a, b) => a < b ? a : b)}%',
            ...
            SizedBox(
              ...
              child: points.isEmpty
                  ? const Center(
                      child: Text(
                        'No battery records for this period.',
  ...
  List<FlSpot> _generateSpots() {
    ...
    if (range == HistoryViewRange.day) {
      return points.map((p) {
        ...
        return FlSpot(hourVal, p.batteryPercentage.toDouble());
      }).toList();
    } else {
      // Week: 0 = 6 days ago, 6 = today
      ...
      return points.map((p) {
        ...
        return FlSpot(diffDays.clamp(0.0, 6.0), p.batteryPercentage.toDouble());
      }).toList();
    }
  }
